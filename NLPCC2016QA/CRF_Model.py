...
class CRF(nn.Module):
    ...
    def _validate(self,
        ...
        if tags is not None:
            no_empty_seq = not self.batch_first and mask[0].all() #  false tensor.all()功能: 如果张量tensor中所有元素都是True, 才返回True; 否则返回False tensor.any()功能: 如果张量tensor中存在一个元素为True, 那么返回True; 只有所有元素都是False时才返回False
    ...
    def _computer_score(self,
        ...
        score = self.start_transitions[tags[0]] # tag[0].shape = [62] 每一句的第一个单词，start到其它tag的得分，随机给一个值
